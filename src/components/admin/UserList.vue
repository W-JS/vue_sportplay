    ...
    <el-card>
      ...
      <el-table :data="userList" border stripe>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <!-- 修改 -->
            <el-button type="primary" icon="el-icon-edit" size="mini" @click="showEditDiglog(scope.row.id)"></el-button>
            <!-- 删除 -->
...
export default {
  ...
  methods: {
    ...
    async userStateChanged(userInfo) {
      const { data:res } = await this.$http.put(`userstate?id=${userInfo.id}&state=${userInfo.state}`);//（已解决）使用put方式访问后台报错,post和get正常
      if(res != "success") {
    ...
    async deleteUser(id){
      ...
      const {data:res} = await this.$http.delete("deleteuser?id=" + id);//（已解决）使用delete方式访问后台报错,post和get正常
      if(res != "success"){
    ...
    edituserInfo(){
      this.$refs.editFormRef.validate(async valid =>{
        if(!valid) return;
        //发起修改的请求
        const {data:res} = await this.$http.put("edituser",this.editForm);//（已解决）使用put方式访问后台报错,post和get正常
